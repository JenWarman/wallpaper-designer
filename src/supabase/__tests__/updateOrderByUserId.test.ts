...
describe("updateOrderByUserId", () => {
  test("it successfully inserts order into order table using user_id", async () => {
    ...
    const insertMock = vi.fn().mockResolvedValue({
      data: [{ id: "user-1", quantity: 4, price: 350.79, design: "DESIGN-1", date: '30/12/2025 '}],
      error: null,
    });
    ...
    const result = await updateOrderByUserId(4, 350.79, 'DESIGN-1', '30/12/2025 ');
  ...
  test("it returns failure when user_id is invalid", async () => {
    ...
    const result = await updateOrderByUserId(4, 350.99, 'DESIGN-1', '30/12/2025 ');
